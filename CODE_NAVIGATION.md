...
| **Kaldığı yerden — ileri pasın BAŞINI kes (2026-09-20, op kutusu, varsayılan KAPALI)** | `start_from_last.py` + `path_generator.py` | Pas NORMAL kurulur (kol + P2 filetosu + çıkış kolu, tüm kendi ayarlarıyla), sonra bir X'ten ÖNCEKİ kısım SİLİNİR → kalan parça sacın içinden başlar. X yazılmaz: `_sfl_anchor` = önceki toolpath'in ucu (`toolpaths[prev_paths_len-1][-1]`), Nokta op'unun `relative` modu ile AYNI çapa. **SADECE İLK PAS** (`i == 0`). **Arama YALNIZCA çıkış kolunda** (`exit_start=split[1]`) — kol neredeyse sabit X'te, tüm yolda arasan KOLUN İÇİNDEN keserdin (test §A bunu ölçüyor). **Kesim geri pas kurulduktan SONRA** — önce kesersen dönüş stroku da kısalır, oysa M'in bütün olayı dönüşün TAM olması. Kesilen pas = TAMAMI çıkış kolu → `last_exit_only_paths` + decimator'da `exit_only=True` dalı (yoksa `exit_max_points` SESSİZCE ölürdü — 2026-08-30'da ters paslarda aynen bu olmuştu). `p2_radius_max_points` uygulanmaz, köşe kesilip gitti. Ret: `no_anchor` / `not_reached` / `too_short` → `last_start_from_last_ignored`. `stop_short` ile birlikte **M harfi**: ileri → kısa geri → kesilmiş ileri → tam geri. UI: `program_tab._add_start_from_last_field` (gri not: SADECE İLK PAS). **KAPALI = bayt bayt eski çıktı.** Test: `_test_start_from_last.py`, `_gui.py` |
