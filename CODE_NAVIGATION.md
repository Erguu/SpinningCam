...
| **Sürekli hareket CMD=2 (2026-09-14, opt-in, DENEYSEL PLC)** | `recipe_to_scl.py` | `continuous_settings(params)` (None = kapalı; `plc_mode` VE `plc_continuous` gerekir) + `plan_continuous_motion(lines, cfg)` (parse_gcode SONUNDA, satır EKLEMEZ/SİLMEZ, F sadece DÜŞER) + `CONTINUOUS_DEFAULTS` + `RecipeLineData.exact` (Nokta op'u → CMD=1 kalır). Köşe ≥ `plc_reversal_deg` (**TAVAN 90°** — üstünde PLC'nin reverse-blocked guard'ı hata verir) → CMD=1; küçük köşe → F = tol/(T·sinθ) aşağı yuvarlanır — **2026-09-16'dan beri köşeye GİREN ve köşeden ÇIKAN satırın İKİSİ de** (`slowed_exits`; dönüşten sonra kayma YENİ satırı sürerken olur). Tam duruştan sonraki satır kapsam DIŞI (duruştan kalkar). F=0 kesme → `CONT_ZERO_FEED:`. UI: `machine_tab.py` `cb_cont` + `_add_cont_entry`; export: `main_window.export_scl_action` `_cont`. **KAPALI = bayt bayt eski çıktı** (7 golden program × işaretli/işaretsiz ile ölçüldü). Mektup: `letter_spinningcam_velocity_path.md`. Test: `_test_continuous_motion.py` (53), `_test_continuous_motion_gui.py` (18) |
| **Takım yolu DÜZENİ — kaç girdi, hangisi geri pas (2026-09-20)** | `path_generator.py` | **ÜÇ AYRI büyüklük, bilerek ayrı — birleştirme:** `op_forward_passes(op)` (İLERİ pas sayısı; **Nokta = 0**, cut/bend = 1, yazılan 0 = 0) · `op_toolpath_stride(op)` (pas başına girdi, 1|2) · `op_toolpath_entries(op)` (op başına = çarpım). `NO_PASS_OP_TYPES` burada; `program_tab` ALIAS eder. Kullananlar: `pass_colors.path_categories`, `program_tab._op_logical_count`/`_op_toolpath_stride`, `main._rtool_for_pass`/`_active_fwd_pass_idx`. **Kural satır içi TEKRARLANMAZ** — 2026-09-20'de iki bug buradan çıktı (Nokta op'u 1 sayılıyordu; `count or 1` yüzünden 0→1). Gerçek referans MOTOR: `len(toolpaths)` + `last_back_pass_meta`. **Golden ağı GÖREMEZ** (sadece ekran: rulo yarıçapı, renk, pas gezgini, PDF). Test: `_test_pass_layout.py` |
| **Kaldığı yerden — ileri pasın BAŞINI kes (2026-09-20, op kutusu, varsayılan KAPALI)** | `start_from_last.py` + `path_generator.py` | Pas NORMAL kurulur (kol + P2 filetosu + çıkış kolu, tüm kendi ayarlarıyla), sonra bir X'ten ÖNCEKİ kısım SİLİNİR → kalan parça sacın içinden başlar. Çapa yazılmaz: `_sfl_anchor` = önceki toolpath'in UCU (`toolpaths[prev_paths_len-1][-1]`), Nokta op'unun `relative` modu ile AYNI çapa. **SADECE İLK PAS** (`i == 0`). **PAS ÖNCE KAYDIRILIR** (`shift_to_meet`, 2026-09-20e): program gereken Başlangıç Z'yi KENDİ hesaplar (`move_dir` = yüzey teğeti `[dr/dz,0,1]`; motorda kaydır→clearance→TEKRAR ÇÖZ, 4 tur). Ne yazarsan yaz AYNI pas; kullanıcının elle bulduğu 12.0 ile fark 0.0045 mm. **Çözücü GERÇEK poliçizgide, en yakın nokta + YEREL teğet** — kirişle çözmek +7.3 sapıyordu (kol EĞRİ: çapa kirişten 6.86, yoldan 1.87 mm). ⚠ **Pasın değdiği yere artık ÖNCEKİ STROK karar verir** → kayma GÖRÜNÜR olmak zorunda: `last_start_from_last_shifts` → pas bilgisi kutusunun başında `pass_info_shifted`. **Kesim EN YAKIN NOKTAYA, X'e DEĞİL** (2026-09-20 düzeltmesi): geri pas ileri pasın TERSİ DEĞİL — `back_pass_arc_z` yayı + kendi clearance kayması onu çizgiden ayırır (sahada 5.6–8.0 mm X). X eşlemek 1.998 mm boşluk bırakıyordu = operatörün girdiği 2.0 mm yay. Sonra `join()` ÇAPAYI yolun önüne koyar → pas rulonun GERÇEK yerinden başlar, kısa birleşme çizgisiyle kendi çizgisine biner. **Pas KAYDIRILMAZ** (clearance'ı ve P3'ü taşırdı). **Arama YALNIZCA çıkış kolunda** (`exit_start=split[1]`) — kol neredeyse sabit X'te, tüm yolda arasan KOLUN İÇİNDEN keserdin (test §A bunu ölçüyor). **Kesim geri pas kurulduktan SONRA** — önce kesersen dönüş stroku da kısalır, oysa M'in bütün olayı dönüşün TAM olması. Kesilen pas = TAMAMI çıkış kolu → `last_exit_only_paths` + decimator'da `exit_only=True` dalı. **GERİ PASLAR da bu kümede** (2026-09-20 saha raporu: tavan onlara HİÇ ulaşmıyordu, 293→49 @ tavan 10); geri pas tanım gereği şekillendirme kolunun tamamı. **Dal NORMAL `tolerance` kullanır, `plc_exit_tolerance` DEĞİL** — exit toleransını tüm stroka uygulamak geri pası 142→3'e düşürüyordu, o ŞEKLİ değiştirir. Tavansız çıktı AYNI. **Üretim programları DEĞİŞMEDİ** (140926/16092026/180926 ölçüldü); ⚠ golden ağı bunu GÖREMEZ, 7 golden dosyasının hiçbiri `exit_max_points` kullanmıyor. `p2_radius_max_points` uygulanmaz, köşe kesilip gitti. Ret: `no_anchor` / `not_reached` / `too_short` → `last_start_from_last_ignored`. `stop_short` ile birlikte **M harfi**: ileri → kısa geri → kesilmiş ileri → tam geri. UI: `program_tab._add_start_from_last_field` (gri not: SADECE İLK PAS). **KAPALI = bayt bayt eski çıktı.** Test: `_test_start_from_last.py`, `_gui.py` |
| **Erken dur — İÇERİ giden stroku kırp (2026-09-20, op alanı, varsayılan KAPALI)** | `stop_short.py` + `path_generator.py` | **KURAL: YÖN, sıra değil** — mandrele DOĞRU giden strok kırpılır. Geri pas (P3→T1) ve TERS pas (P3→kol başı) EVET; İLERİ pas ASLA (o uç `reach` / sac çarpanının işi). Takas AÇIKSA da kırpılan içeri giden olur → bu yüzden kod swap bloğundan SONRA, `bck_path` üzerinde çalışır (`_bp_path`'i yukarıda kırpmak takasta DIŞA gideni keserdi). Saf: `plan(path, mm)` → `(keep, t)` (mesafe YOL BOYUNCA; son nokta interpolasyonla tam yerinde), `apply`, `apply_parallel` (proj/dev; **motorda ZATEN 1 eksik proj var** — linear_approach'ta 20 vs 21, bu özellikten ÖNCE de vardı, bu yüzden `n` veya `n-1` kabul edilir). Ters dalda `last_reverse_split_idx` **kırpma ÖNCESİ uzunlukla** hesaplanır + clamp. Reddetme: strok < kırpma → `last_stop_short_ignored` (BİLDİRİLİR, güdük pas GÖNDERİLMEZ). **Mandrel ucu bağlantısını KAPATIR** (`mandrel_end_link` yeni sebep `stop_short`: pas artık mandrelde bitmiyor). UI: `program_tab._add_stop_short_field` (gri not + 15 karakter sınırı), i18n `lbl_stop_short` / `note_stop_short`. **KAPALI = bayt bayt eski çıktı.** Test: `_test_pass_end_trim.py`, `_gui.py` |
| **Mandrel ucunda geri çekilme yok (2026-09-16, op kutusu, varsayılan KAPALI)** | `mandrel_end_link.py` + `path_generator.py` | Kural: `blockers()` + `check_geometry()` (saf). Karar BİR KEZ: `calculate_paths` → `_apply_mandrel_end_links()` (mirror'dan önce; toolpath listesi değişmez, sim sequence'ı yeniden yazılır) → `last_mandrel_links` / `last_mandrel_link_refused`. Yazıcı: `generate_gcode` geri çekilmeyi TUTAR (`_held_retract`), B başında `is_machine_line` → geri koy (`last_mandrel_link_fallbacks`) ya da `(Link Op` G1 / hiç satır. Reçete: `(Link Op` → `exact` (CMD=1). 3B: `main.py` gri çizgi. UI: `program_tab._add_mandrel_link_fields`. **1 duruş istenirse:** bağlantı G1'ini pas başlığından önce yaz. Test: `_test_mandrel_end_link.py`, `_gui.py` |
| **Eğride UZUN satır uyarısı (2026-09-16)** | `short_segments.py` | `analyze_long(path_gen, T, params)`: uzunluk > 2×max(2.5, 5·F·T) VE kiriş hatası > 0.05 mm (`_full_indices` + `_chord_error`; düz çizgi asla). Öneri: dolu sınır → sınır; değilse tolerans `_trial_tolerance` ile (path_gen kopyasında yeniden inceltme); auto-tune → satır hedefi. `format_long_report`. Aynı pencere: `main_window._confirm_short_segments(cont, _xp)`. Metinler sade (operatör). **Renkli pencere:** `ui/dialogs/line_length_dialog.py` (`classify`/`head_lines` satır rolü; yeşil = öneri, YENİ değer ayrıca vurgulu). Test: `_test_long_chords.py`, `_test_line_length_dialog.py` |
| **HİÇBİR YERE GİTMEYEN rapid atılır (2026-09-18)** | `path_generator.py` | `drop_zero_length_rapids(lines)` — saf, `generate_gcode`'un SONUNDA. Takımın ZATEN durduğu noktaya giden `G0` yazılmaz; eksen bazında modal, adını andığı HER eksen (B dahil) o değerdeyse atılır. Besleme satırı ASLA; konum bilinmeden ASLA (program başı referansları kalır). **Takım değişimi güvenlik bloğu MUAF** (kullanıcı, 2026-09-19): `TOOL_CHANGE_SAFETY_HEADER` TEK sabit — emitter yazar, atıcı okur, başlık değişirse koruma sessizce kalkamaz. ⚠ Bu muafiyet KANITA DAYANMIYOR, güvenli taraf seçildi — **AÇIK SORU, TODO #108**: satır bir hareket mi, yoksa her seferinde tekrarlanan bir SÖZ mü? Ölçüm: 140926 → 1 atılır + 1 korunur; 180926 → 2 korunur, HİÇBİRİ atılmaz. Sayaç: `last_zero_rapids_dropped`. Mektup: `letter_spinningcam_zero_length_rapid.md`. Test: `_test_zero_rapids.py` |
| **Kısa satır uyarısı (2026-09-15, sadece sürekli hareket AÇIK)** | `short_segments.py` | `analyze(path_gen, T)` — `last_plc_paths` + `last_path_point_feeds` (generate_gcode'un KAYDI, çıktıya dokunmaz) okur; önerilen uzunluk 5×F×T; bölüm T1/T2 konumundan; öneri floor(L÷önerilen)+1 en kısa pasta. `format_report(reports, t, ...)`. UI: `main_window._confirm_short_segments`. **Deneme koşusu YOK** — değer girilince #99/#101 kapısı yargılar. Test: `_test_short_segments.py` (32) |
...
> **⚠ 0.012 mm KURALI (2026-09-16, kullanıcı kararı + PLC ekibi reply3).** `decimate_all_paths` SONUNDA
> `_drop_microsegments` (sabit `MICRO_SEGMENT_MM = 0.012`) bir önceki KORUNAN noktaya
> 0.012 mm'den yakın noktayı ATAR. **Neden 0.012:** PLC testi `<= 0.01` ve float32 —
> tam 0.01000 bizden geçip orada reddedilirdi (PLC ekibi, reply3). **SADECE hız modunda** (`plc_mode` VE
