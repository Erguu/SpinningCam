...
# SABİT KONUMLAR VE RULO OFSETLERİ
FIXED_BLANK_CENTER_X = 0.0
FIXED_BLANK_CENTER_Y = 0.0
FIXED_BLANK_BOTTOM_Z = 0.0 # Sacın alt yüzeyinin Z konumu (örneğin tabla seviyesi)

ROLLER_BODY_RADIUS_VIS = 25.0 # Rulo gövde yarıçapı (önceki main'deki ile aynı)
ROLLER_X_OFFSET_FROM_PART = 30.0 # Rulonun parçanın kenarından X'teki mesafesi
ROLLER_Z_OFFSET_ABOVE_PART = 30.0  # Rulonun parçanın üstünden Z'deki mesafesi
...
dynamic_roller_visual_actor = None # DİNAMİK RULO GÖRSELİ İÇİN YENİ AKTÖR
...
def get_radius_at_z(shape_to_section, z_level, center_xy=gp_Pnt(0,0,0)):
    ...
    try:
        ...
        return 0.0
def reorient_shape_and_get_properties(shape, flip_z=False, target_top_z=0.0, target_xy_center_coords=(0.0, 0.0)):
    ...
    x_min_i, y_min_i, z_min_i, x_max_i, y_max_i, z_max_i = bbox_initial.Get()

    oriented_shape = shape # Başlangıçta şeklin kendisi
    
    if not flip_z:
        # Mandrelin orijinal üst yüzeyini (z_max_i) target_top_z'ye getir.
        # Mandrelin orijinal XY merkezini target_xy_center_coords'a getir.
        trsf_vec = gp_Vec(target_xy_center_coords[0] - center_initial.X(),
                          target_xy_center_coords[1] - center_initial.Y(),
                          target_top_z - z_max_i)
        trsf_align = gp_Trsf()
        trsf_align.SetTranslation(trsf_vec)
        oriented_shape = BRepBuilderAPI_Transform(oriented_shape, trsf_align, True).Shape()
    else: # flip_z True ise
        print("INFO: Mandrel Z ekseninde çevriliyor...")
        # Mandrelin orijinal alt yüzeyini (z_min_i) target_top_z'ye getir (bu yeni üst olacak).
        # Mandrelin orijinal XY merkezini target_xy_center_coords'a getir.
        trsf_vec_for_flip_base = gp_Vec(target_xy_center_coords[0] - center_initial.X(),
                                        target_xy_center_coords[1] - center_initial.Y(),
                                        target_top_z - z_min_i) # Orijinal altı hedefe taşı
        trsf_align_for_flip = gp_Trsf()
        trsf_align_for_flip.SetTranslation(trsf_vec_for_flip_base)
        temp_oriented_shape = BRepBuilderAPI_Transform(oriented_shape, trsf_align_for_flip, True).Shape()

        # Şimdi bu şekli, hedef X,Y,Z noktasından geçen X ekseni etrafında 180 derece çevir.
        # Bu, z_min_i'nin olduğu yüzeyi yukarı getirecek ve target_top_z'de tutacak.
        # Çevirme merkezi: (target_xy_center_coords[0], target_xy_center_coords[1], target_top_z)
        # Bu merkez, aynı zamanda temp_oriented_shape'in (eski z_min_i) yeni üst yüzeyinin merkezi olacak.
        
        # ÖNEMLİ: Çevirme işlemi için, geçici olarak hizalanmış şeklin X,Y merkezini kullanmak daha doğru olabilir.
        # bbox_temp, center_temp = get_shape_bounds_and_center(temp_oriented_shape)
        # rotation_point = gp_Pnt(center_temp.X(), center_temp.Y(), target_top_z)
        # VEYA doğrudan hedef koordinatları kullanalım:
        rotation_point = gp_Pnt(target_xy_center_coords[0], target_xy_center_coords[1], target_top_z)
        
        rotation_axis = gp_Ax1(rotation_point, gp_Dir(1, 0, 0)) # X ekseni etrafında çevir
        trsf_flip_action = gp_Trsf()
        trsf_flip_action.SetRotation(rotation_axis, math.pi) # 180 derece
        oriented_shape = BRepBuilderAPI_Transform(temp_oriented_shape, trsf_flip_action, True).Shape()

    # Son özellikleri hesapla
    final_bbox, final_center = get_shape_bounds_and_center(oriented_shape)
    if final_bbox is None:
        print("HATA: Yönlendirilmiş şeklin sınırlayıcı kutusu alınamadı.")
        return oriented_shape, 0, 0, 0, 0

    fx_min, fy_min, fz_min_final, fx_max, fy_max, fz_max_final = final_bbox.Get()
    # fz_max_final şimdi target_top_z'ye çok yakın olmalı.
    
    est_h = fz_max_final - fz_min_final # Mandrelin gerçek yüksekliği
    
    # Kesit için XY merkezi (artık target_xy_center_coords)
    shape_xy_center_for_sectioning = gp_Pnt(target_xy_center_coords[0], target_xy_center_coords[1], 0) 
    
    base_z_for_radius_calc = fz_min_final + 0.01 * est_h if est_h > 1e-3 else fz_min_final + 0.01
    est_br = get_radius_at_z(oriented_shape, base_z_for_radius_calc, shape_xy_center_for_sectioning)
    
    top_z_for_radius_calc = fz_max_final - 0.01 * est_h if est_h > 1e-3 else fz_max_final - 0.01
    est_tr = get_radius_at_z(oriented_shape, top_z_for_radius_calc, shape_xy_center_for_sectioning)

    if est_h < 0.1: est_tr = est_br
    if est_br < 1e-2: 
        print(f"UYARI: Tahmini taban yarıçapı kesitle çok küçük: {est_br:.3f}. BBox X/Y max kullanılacak.")
        est_br = max(abs(fx_min - target_xy_center_coords[0]), abs(fx_max - target_xy_center_coords[0]), 
                     abs(fy_min - target_xy_center_coords[1]), abs(fy_max - target_xy_center_coords[1]))
            
    if est_tr < 1e-2 and est_h >= 0.1 : # Sadece gerçekten bir yüksekliği varsa ve tepe radyanı çok küçükse
        print(f"UYARI: Tahmini tepe yarıçapı kesitle çok küçük: {est_tr:.3f}. BBox X/Y max * 0.1 kullanılacak.")
        est_tr = max(abs(fx_min - target_xy_center_coords[0]), abs(fx_max - target_xy_center_coords[0]), 
                     abs(fy_min - target_xy_center_coords[1]), abs(fy_max - target_xy_center_coords[1])) * 0.1
        if est_tr < 1e-2: est_tr = 0.0

    print(f"Yönlendirilmiş Mandrel (Hedeflenen Üst Z: {fz_max_final:.2f} ~ {target_top_z:.2f}): Yükseklik={est_h:.2f}, Taban R={est_br:.2f}, Tepe R={est_tr:.2f}")
    # İkinci dönen değer, mandrelin yönlendirilmiş haldeki üst yüzeyinin Z'sidir.
    # Bu Z, sacın altının oturacağı yer olacak.
    return oriented_shape, fz_max_final, est_h, est_br, est_tr
...
def calculate_and_draw_all(plotter_obj_ref):
    ...
    global dynamic_roller_visual_actor # YENİ AKTÖRÜ EKLE
# Önceki dinamik çizimleri temizle
    ...
    if dynamic_roller_visual_actor: # DİNAMİK RULO GÖRSELİNİ TEMİZLE
        plotter_obj_ref.remove_actor(dynamic_roller_visual_actor, render=False)
        dynamic_roller_visual_actor = None
...
# 1. Mandreli Yeniden Yönlendir ve Boyutlarını Al (SABİT SACA GÖRE)
    # Mandrelin üst yüzeyi, sacın alt yüzeyinin Z'sine (FIXED_BLANK_BOTTOM_Z) gelecek.
    # Mandrelin XY merkezi, sacın XY merkezine (FIXED_BLANK_CENTER_X/Y) gelecek.
    current_mandrel_occ, mandrel_world_top_z, current_m_h, current_m_br, current_m_tr = \
        reorient_shape_and_get_properties(raw_mandrel_shape, params["flip_z_axis"],
                                          target_top_z=FIXED_BLANK_BOTTOM_Z,
                                          target_xy_center_coords=(FIXED_BLANK_CENTER_X, FIXED_BLANK_CENTER_Y))
    
    if current_mandrel_occ is None: # Hata kontrolü
...
# Levha Mesh'ini Güncelle (SABİT KONUMDA)
    if blank_actor: plotter_obj_ref.remove_actor(blank_actor, render=False)
    # Sacın alt yüzey merkezi FIXED_BLANK_BOTTOM_Z'de olacak.
    # BRepPrimAPI_MakeCylinder merkez olarak taban merkezini alır ve yukarı doğru oluşturur.
    blank_base_center_fixed = gp_Pnt(FIXED_BLANK_CENTER_X, FIXED_BLANK_CENTER_Y, FIXED_BLANK_BOTTOM_Z)
    blank_placement_fixed = gp_Ax2(blank_base_center_fixed, gp_Dir(0,0,1)) # Z ekseni boyunca
    current_blank_occ = BRepPrimAPI_MakeCylinder(blank_placement_fixed, blank_radius, blank_thickness).Shape() 
    ...
    # ... (blank_actor oluşturulduktan HEMEN SONRA)

        # --- Rulo Bekleme Konumunu Hesapla (SABİT SACA GÖRE) ---
        # Bu hesaplama, rulonun DÜNYA KOORDİNATLARINDAKİ SABİT yerini belirler.
        # Girdiler (FIXED_BLANK_..., blank_radius, ROLLER_...) sabit olduğu için sonuç da sabit olur.

        # Rulonun X konumu: Sabit sacın X merkezinden, sacın yarıçapı kadar ve tanımlı ofsetler kadar dışarıda.
        dynamic_roller_pos_x = FIXED_BLANK_CENTER_X + blank_radius + ROLLER_BODY_RADIUS_VIS + ROLLER_X_OFFSET_FROM_PART
        
        # Sabit sacın üst yüzeyinin Z'si
        fixed_blank_top_surface_z = FIXED_BLANK_BOTTOM_Z + blank_thickness
        # Rulonun Z konumu: Sabit sacın üst yüzeyinden ve tanımlı ofsetler kadar yukarıda.
        dynamic_roller_pos_z = fixed_blank_top_surface_z + ROLLER_BODY_RADIUS_VIS + ROLLER_Z_OFFSET_ABOVE_PART

        # --- Sabit Konumlu Rulo Görselini Oluştur ve Ekle ---
        # Not: dynamic_roller_visual_actor'un temizlenmesi zaten fonksiyonun başındaki temizleme bölümünde yapılıyor olmalı.
            
        roller_length_vis_local = 40.0 # Rulo boyu, sabit veya parametrik olabilir
        roller_visual_tilt_degrees_around_x_local = 0.0 # Rulo eğimi, şimdilik 0

        # Rulonun merkez noktası (Y'si sacın Y merkeziyle aynı)
        roller_center_pnt_vis_fixed = gp_Pnt(dynamic_roller_pos_x, FIXED_BLANK_CENTER_Y, dynamic_roller_pos_z)
        
        roller_main_axis_vis_dir = gp_Dir(0,1,0) # Rulonun ana ekseni Y boyunca (dönme ekseni)
        
        # Eğer rulo eğimi varsa (roller_visual_tilt_degrees_around_x_local != 0),
        # roller_main_axis_vis_dir buna göre güncellenmeli. Bu kısım şimdilik atlanmıştır (tilt=0).
        # Örnek: gp_Ax2(merkez, Z_yonu, X_referans_yonu) -> silindir Z boyunca uzar.
        # Bizim rulo Y ekseni boyunca uzuyor.

        ref_direction_for_ax2_roller = gp_Dir(1,0,0) # X ekseni, gp_Ax2 için referans yönü
        if roller_main_axis_vis_dir.IsParallel(ref_direction_for_ax2_roller, 1e-5): # Ana eksen X'e paralelse
            ref_direction_for_ax2_roller = gp_Dir(0,0,1) # Referansı Z yap

        try:
            roller_placement_vis_fixed = gp_Ax2(roller_center_pnt_vis_fixed, roller_main_axis_vis_dir, ref_direction_for_ax2_roller)
        except RuntimeError: # Nadir bir paralellik durumu için alternatifler
            alt_ref_dir = gp_Dir(0,0,1) if not roller_main_axis_vis_dir.IsParallel(gp_Dir(0,0,1),1e-5) else gp_Dir(1,0,0)
            try:
                roller_placement_vis_fixed = gp_Ax2(roller_center_pnt_vis_fixed, roller_main_axis_vis_dir, alt_ref_dir)
            except RuntimeError: # En son çare, sadece merkez ve ana yön
                roller_placement_vis_fixed = gp_Ax2(roller_center_pnt_vis_fixed, roller_main_axis_vis_dir)             

        roller_shape_vis_occ_fixed = BRepPrimAPI_MakeCylinder(roller_placement_vis_fixed, ROLLER_BODY_RADIUS_VIS, roller_length_vis_local).Shape()
        pv_roller_vis_fixed = occ_to_pyvista(roller_shape_vis_occ_fixed)
        
        if pv_roller_vis_fixed:
            # dynamic_roller_visual_actor global değişkenine atama yapıyoruz.
            dynamic_roller_visual_actor = plotter_obj_ref.add_mesh(pv_roller_vis_fixed, name="roller_visual_fixed_pos", color='darkgoldenrod', smooth_shading=True)
        # --- Sabit Konumlu Rulo Görseli Sonu ---

        # --- Takım Yolu Hesaplamaları (current_m_h, current_m_br, current_m_tr kullanarak) ---
        # (Bu yorum satırı zaten kodunuzda olmalı, rulo oluşturma bunun hemen üstüne gelecek)
    ...
    for p_idx in range(num_sweeping_passes):
        # Mandrelin DÜNYADAKİ gerçek Z sınırları:
        # mandrel_world_top_z, reorient_shape_and_get_properties'ten gelen ve FIXED_BLANK_BOTTOM_Z'ye eşit olan değerdir.
        mandrel_actual_world_top_z = mandrel_world_top_z 
        mandrel_actual_world_base_z = mandrel_world_top_z - current_m_h
        ...
        # P2 için hedef Z'yi belirle (DÜNYA Z KOORDİNATLARINDA)
        target_z_for_p2_this_pass_contact = 0.0 # Başlangıç değeri
# ... (total_offset_distance hesaplandıktan sonra) ...
        if p_idx == 0: # İlk paso
            if first_pass_p2_z_override is not None: # GUI'den gelen MUTLAK Z değeri (first_pass_p2_contact_z_abs)
                # Değeri mandrelin dünya Z sınırları içinde kısıtla
                target_z_for_p2_this_pass_contact = max(mandrel_actual_world_base_z + dz_small_for_normal,
                                                        min(first_pass_p2_z_override, mandrel_actual_world_top_z - dz_small_for_normal))
            else: 
                # Override yoksa, ilk pasonun ortası, mandrelin kendi tepesinden yüksekliğinin çeyreği kadar aşağıda olsun (örnek bir mantık)
                # (current_m_h / num_sweeping_passes) -> bir paso segmentinin yaklaşık yüksekliği
                # Bunun yarısı -> segmentin ortası
                z_offset_from_mandrel_top = (0.5 * (current_m_h / num_sweeping_passes)) if num_sweeping_passes > 0 else (0.5 * current_m_h)
                target_z_for_p2_this_pass_contact = mandrel_actual_world_top_z - z_offset_from_mandrel_top
        else: # Sonraki pasolar (p_idx > 0)
            # Bu kısım, pasoların Z'de nasıl dağılacağına dair sizin mantığınıza göre ayarlanmalı.
            # Örnek: Kalan yüksekliği eşit bölerek ilerleme.
            # Her pasonun merkezinin, mandrelin toplam yüksekliği içindeki göreceli konumu:
            # (p_idx + 0.5) / num_sweeping_passes  -> paso merkezinin normalize edilmiş konumu (0 en üst, 1 en alt gibi)
            # Bu değeri mandrelin toplam yüksekliğiyle çarpıp mandrelin tepesinden çıkarırız.
            current_pass_center_fraction = (p_idx + 0.5) / num_sweeping_passes
            target_z_for_p2_this_pass_contact = mandrel_actual_world_top_z - (current_pass_center_fraction * current_m_h)
            
            # Eğer daha önceki `z_start_of_current_segment_group` mantığınızı korumak isterseniz:
            # Orijinal kodunuzda:
            # num_remaining_passes = num_sweeping_passes - p_idx
            # remaining_height = z_start_of_current_segment_group # Bu, mandrelin KENDİ TEPESİNE GÖRE kalan yükseklik olmalıydı.
            # segment_h_for_remaining = remaining_height / num_remaining_passes if num_remaining_passes > 0 else remaining_height
            # current_pass_segment_top_z_contact_relative = z_start_of_current_segment_group
            # current_pass_segment_bottom_z_contact_relative = z_start_of_current_segment_group - segment_h_for_remaining
            # target_z_relative_to_mandrel_top = (current_pass_segment_top_z_contact_relative + current_pass_segment_bottom_z_contact_relative) / 2.0
            # target_z_for_p2_this_pass_contact = mandrel_actual_world_top_z - target_z_relative_to_mandrel_top 
            # Bu durumda z_start_of_current_segment_group'un doğru şekilde güncellendiğinden emin olmalısınız.
            # (Aşağıdaki z_start_of_current_segment_group güncellemesini de buna göre ayarlamanız gerekir.)

        # Hesaplanan P2 Z temas noktasını mandrel sınırları içinde son bir kez daha kısıtla:
        target_z_for_p2_this_pass_contact = max(mandrel_actual_world_base_z + dz_small_for_normal, 
                                                min(target_z_for_p2_this_pass_contact, mandrel_actual_world_top_z - dz_small_for_normal))
    ...
    if final_control_points_to_plot and isinstance(final_control_points_to_plot, list) and len(final_control_points_to_plot) > 0:
        # Rulonun yaklaşma hareketinin başlayacağı X ve Z koordinatları:
        # Bunlar yukarıda hesaplanan, saca göre sabit olan rulo bekleme konumudur.
        initial_roller_x_for_approach = dynamic_roller_pos_x
        initial_roller_z_for_approach = dynamic_roller_pos_z

        first_P1 = final_control_points_to_plot[0][0] # (X, Y, Z)
        # Rulonun başlangıç 3D konumu (Y'si ilk P1 ile aynı, X ve Z'si sabit bekleme konumu)
        current_roller_pos = np.array([initial_roller_x_for_approach, first_P1[1], initial_roller_z_for_approach])
