...
class ProgramTab:
    ...
    def _add_tool_change_fields(self, idx, op):
        ...
        Shared by EVERY op type. Cutting/bending render it too (2026-07-31): the
        engine has always honored these keys for them — `resolve_tool_change_point`
        runs before the type check in both calculate_paths and generate_gcode — but
        their editor branch returned before this block, so the fields existed in
        OP_PARAM_UNIVERSE (columnable in Customize View) with nothing to edit them.
        These are usually the ops that TRIGGER the change, since they bring their
        own tool.
        """
            ...
            if is_linear:
                self._add_prop_entry(idx, "p2_radius", t("lbl_p2_radius"), op, is_float=True,
                                     tooltip="P2 köşesinde gerçek sabit-yarıçaplı bir daire yayı (fileto) oluşturur. "
                                             ...
                                             "Tipik: 5–15 mm.")
                # #99: fillet point cap — PLC/SCL reçetesindeki nokta sayısını sınırlar.
                self._add_prop_entry(idx, "p2_radius_max_points", t("lbl_p2_max_pts"), op,
                                     is_int=True, default_hint="= ∞",
                                     tooltip="P2 fileto yayının PLC reçetesinde kaç noktaya kadar "
                                             "inebileceği (en fazla). BOŞ/0 = sınır yok (varsayılan, "
                                             "bugünkü davranış).\n"
                                             "NEDEN: tezgah her noktada durup hızlanıyor; sık noktalı "
                                             "fileto yavaş ve kesik kesik çalışıyor. Az nokta = daha "
                                             "akıcı hareket, daha kaba köşe.\n"
                                             "⚠️ SADECE PLC/SCL çıktısını etkiler — 3B görünüm ve normal "
                                             ".nc dosyası tam çözünürlükte kalır, yani ekranda bir "
                                             "değişiklik GÖRMEZSİNİZ. Etkisi SCL İnceleme penceresinde "
                                             "görülür.\n"
